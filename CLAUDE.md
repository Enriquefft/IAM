...
Monorepo holding both the **software we ship** (products under `product/`, all code workspaces under `apps/`) and the **business intelligence source of truth** (everything non-code).
...
- `business/` — market, competitive, positioning, personas, problems, pricing
- `clinical/` — autism science, evidence base, interventions, AAC standards, accessibility, regulation. Healthtech evidence layer; primary reader is Claude Code.
- `product/` — per-product spec, MSP, UX decisions (one subdir per product)
- `company/` — legal, finance, captable, partnerships, team, grants, internal meetings
- `apps/` — all code workspaces (engineering deep-dives live here under `apps/research/` once a product is chosen)
